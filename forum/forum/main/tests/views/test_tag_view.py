...
class TagViewTests(TestCase):
    ...
    def test_get__should_render_template(self):
        tag = Tag.objects.create(**self.VALID_TAG_DATA)
        response = self.client.get(reverse('tag view', args=[tag.pk]))
        self.assertTemplateUsed(response, 'main/tag.html')

    def test_non_existent_tag__expect_404(self):
        tag = Tag.objects.create(**self.VALID_TAG_DATA)
        response = self.client.get(reverse('tag view', args=[2]))
        self.assertEqual(response.status_code, 404)

    def test_get__tag_name__expect_tag_name(self):
        tag = Tag.objects.create(**self.VALID_TAG_DATA)
        response = self.client.get(reverse('tag view', args=[tag.pk]))
